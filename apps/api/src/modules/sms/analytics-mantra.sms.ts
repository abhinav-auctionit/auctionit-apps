...
export class AnalyticsMantraSmsService implements SmsService {
  ...
  async send({ to, message }: SmsSendInput): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `SMS gateway request failed: ${err instanceof Error ? err.message : err}`,
      );
    ...
    if (!res.ok) {
      this.logger.error(`SMS gateway HTTP ${res.status}: ${body.slice(0, 200)}`);
    ...
    if (!body.includes(this.cfg.successToken)) {
      this.logger.error(`SMS gateway did not confirm success: ${body.slice(0, 200)}`);
      throw new Error('SMS gateway returned non-success response');
    }

    this.logger.log(`SMS dispatched to=${mobile} response=${body.slice(0, 80)}`);
  }
}
